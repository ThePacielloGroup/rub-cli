...
commands.create({
  ...
  expandMimeTypes () {
    for (const mimeType in this._mimeTypes) {
      this._mimeTypes[mimeType].forEach((fileExtension) => {
        this._mimeTypesByExtension[ fileExtension ] = mimeType
      })
    }
  },
  ...
  injectScript (req, res) {
    let fileString = fs.readFileSync(req.stat.filename).toString()

    const start = fileString.toLowerCase().indexOf('<head>')
    if (start > -1) {
      const begin = fileString.substr(0, start)
      const end = fileString.substr(start + 6)
  ...
  next (req, res) {
    const mimeType = this._mimeTypesByExtension[path.extname(req.stat.filename).split('.')[1]]
    if (mimeType === 'text/html') {
      let injectScript = false
      if (req.headers.referer !== undefined) {
        const stat = this.urlStat(req.headers.referer)
  ...
  request (req, res) {
    ...
    if (stat.isDirectory()) {
      return this.serveIndex(req, res, (req, res) => {
        this.next(req, res)
      })
    }
    this.next(req, res)
  ...
  reload (type) {
    if (!this._isStarted) return
    notice("Server sending client reload action '" + type + "'")
